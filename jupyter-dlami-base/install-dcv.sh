...
# but note that our alinux2 will install XFCE instead of GNOME3.
...
# Uncomment FONTS variable to install all fonts, but beware for a longer installation time.
# declare -a FONTS=(
...
1) Ensure this instance's role permits S3 public endpoint, as dcv needs to check for a specific
   bucket for licensing.
   - It is still free to use on EC2, just that it needs to ping S3.
   - See https://docs.aws.amazon.com/dcv/latest/adminguide/setting-up-license.html
2) Reboot
